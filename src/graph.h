...
class Graph {
  ...
  protected:
	...
	std::map<int, std::list<Vertex*> > degreesList;
  ...
  public:
	Graph(bool m_simple = true, bool m_weighted = false);
	~Graph();
	...
	inline std::map<int, std::list<Vertex*> >& getDegreesList() { return degreesList; };
