...
Graph::Graph(bool m_simple, bool m_weighted) {
	simple = m_simple;
	weighted = m_weighted;
...
void Graph::buildDegreesList() {
	...
	for (it=vertices_map.begin();it!=vertices_map.end();it++) {
		degreesList[(it)->second->getDegree()].push_back((it)->second);
	}
}
...
void Graph::printDegreesList() {
	...
	std::map<int, std::list<Vertex*> >::iterator it;
...
//	if (it1==vertices_map.end()) {
	if (vertices_map.find(vn->getNumber())==vertices_map.end()) {
		vertex = new Vertex(vn->getNumber());
		totalVertices++;
...
void Graph::addNodesToConnectedComponent(ConnectedComponent* comp, Vertex* v) {
	mark[v]=1;
	comp->addVertex(v);
	std::vector<Vertex*>::iterator neighbours = v->getNeighbours()->begin();
	while (neighbours != v->getNeighbours()->end()) {
		if (mark.find(*neighbours)==mark.end())
			addNodesToConnectedComponent(comp, *neighbours);
		neighbours++;
	}
};
